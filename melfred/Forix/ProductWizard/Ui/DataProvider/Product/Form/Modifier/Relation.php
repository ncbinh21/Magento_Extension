...
namespace Forix\ProductWizard\Ui\DataProvider\Product\Form\Modifier;
...
class Relation extends AbstractModifier
{
    ...
    const DATA_SCOPE_RELATION = \Forix\ProductWizard\Model\ResourceModel\Product\Link::LINK_TYPE_RELATION_CODE;

    const GROUP_LINKED = \Forix\ProductWizard\Model\ResourceModel\Product\Link::LINK_TYPE_RELATION_CODE . '_product_selection';
    ...
    public function __construct(
        ...
        \Forix\ProductWizard\Model\Product\LinkOptions\Required $required,
        $scopeName = '',
        $scopePrefix = ''
    )
    {
    ...
    public function modifyMeta(array $meta)
    {
        $meta = array_replace_recursive(
            ...
            [
                static::GROUP_LINKED => [
                    'children' => [
                        $this->scopePrefix . static::DATA_SCOPE_RELATION => $this->getRelationFieldset(),
                    ],
                    'arguments' => [
                        'data' => [
                            'config' => [
                                'label' => __('Linked Relation Products'),
                                'collapsible' => true,
     ...
     * @return array
     */
    protected function getRelationFieldset()
    {
        $content = __(
            'Select Linked Relation Products'
        );
        $fieldSet = [
            'children' => [
                'button_set' => $this->getButtonSet(
                    $content,
                    __('Select Relation Product'),
                    $this->scopePrefix . static::DATA_SCOPE_RELATION
                ),
                'modal' => $this->getGenericModal(
                    __('Select Relation Product'),
                    $this->scopePrefix . static::DATA_SCOPE_RELATION
                ),
                static::DATA_SCOPE_RELATION => $this->getGrid($this->scopePrefix . static::DATA_SCOPE_RELATION),
            ],
            'arguments' => [
                'data' => [
                    'config' => [
                        'additionalClasses' => 'admin__fieldset-section',
                        'label' => __('Relation Product'),
                        'collapsible' => false,
                        'componentType' => Fieldset::NAME,
                        'dataScope' => '',
                        'sortOrder' => 15,
    ...
    {
        $modalTarget = $this->scopeName . '.' . static::GROUP_LINKED . '.' . $scope . '.modal';
