 ...
 */
namespace Forix\Configurable\Rewrite\Magento\ConfigurableProduct\Model\Order\Reorder;
 ...
 */
class OrderedProductAvailabilityChecker extends \Magento\ConfigurableProductSales\Model\Order\Reorder\OrderedProductAvailabilityChecker
{
    /**
    ...
    public function isAvailable(Item $item)
    {
        ...
        $orderItemParentId = $item->getParentItem()->getProduct()->getRowId();
        $select->from(
        ...
        foreach ($superAttribute as $attributeId => $attributeValue) {
            $rigModel = $item->getBuyRequest()->getRigModel();
            if($rigModel && isset($rigModel[$attributeId])){
                $select->join(
                    ['cpid' . $attributeId => $this->resourceConnection->getTableName('catalog_product_entity_varchar')],
                    sprintf(
                    ...
                    ->joinLeft(
                        ['cpis' . $attributeId => $this->resourceConnection->getTableName('catalog_product_entity_varchar')],
                        sprintf(
