...
namespace Forix\Bannerslider\Model\ResourceModel\Banner;
...
{
    /**
     * @var \Forix\Bannerslider\Helper\Mobiledetect
     */
    ...
    {
        $this->_init('Forix\Bannerslider\Model\Banner', 'Forix\Bannerslider\Model\ResourceModel\Banner');
    }
    ...
    public function __construct(
        ...
        \Forix\Bannerslider\Helper\Mobiledetect $mobiledetect,
        $connection = null,
    ...
    public function joinSliderTitle()
    {
        $this->getSelect()->joinLeft(
            ['sliderTable' => $this->getTable('forix_bannerslider_slider')],
    ...
    public function addFieldToFilter($field, $condition = null)
    {
        ...
        if (in_array($field, $attributes) && $storeViewId) {
            if (!in_array($field, $this->_addedTable)) {
                ...
                $this->getSelect()
                    ->joinLeft(array($field => $this->getTable('forix_bannerslider_value')), $sql, array());
    ...
    {
        parent::_afterLoad();
        $isTable = $this->_mobileDetect->isTablet();
        ...
        if($isTable){
            foreach ($this->_items as $item) {
                if($item->getTablet() != ''){
                    $item->setImage($item->getTablet());
                    $item->setImageThumb($item->getTabletThumb());
                }
            }
        }else if($isMobile){
            foreach ($this->_items as $item) {
                if($item->getPhone() != ''){
                    $item->setImage($item->getPhone());
                    $item->setImageThumb($item->getPhoneThumb());
                }
            }
        }*/
        ...
        foreach ($this->_items as $item) {
            //keep desktop images
            $item->setDesktop($item->getImage());
            $item->setDesktopThumb($item->getImageThumb());
            
            if($isTable && $item->getTablet() != ''){
                $item->setImage($item->getTablet());
                $item->setImageThumb($item->getTabletThumb());
            }
            else if($isMobile && $item->getPhone() != ''){
                $item->setImage($item->getPhone());
                $item->setImageThumb($item->getPhoneThumb());
            }
