...
namespace Forix\Catalog\Block\Product\View\Options\Type;
	...
	public function getValuesHtml()
	{
		...
		if ($_option->getType() == \Magento\Catalog\Api\Data\ProductCustomOptionInterface::OPTION_TYPE_RADIO ||
			...
			foreach ($_option->getValues() as $_value) {
				...
				$_items = $this->getProductBySku($_value->getSku());
				$_thumbnail = "";
				if($_items->getThumbnail()!='') {
					$_thumbnail = $this->baseHelper->getMediaUrl().'catalog/product'.$_items->getThumbnail();
				}

				$selectHtml .= '<div class="field choice admin__field admin__field-option' .
					$require .
					'">' .
					'<input data-image="'.$_thumbnail.'" type="' .
					$type .
					'" class="' .
					$class .
					' ' .
					...
					'" />' .
					'<label class="label admin__field-label" for="options_' . $_option->getId() . '_' . $count . '"><span>' .
					$_value->getTitle() . ' (+'. $this->pricingHelper->currencyByStore($_value->getPrice(true), $store, true) . ')' .
					'</span> ' .
