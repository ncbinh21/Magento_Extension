...
class ConfigurableVariationLabels extends AbstractColumnType
{
    /**
    ...
    {
        if (isset($rawData[$this->_productTypeCol])) {
                ...
                if (in_array( $rawData[$this->_columnByAttributeCode[ImportProduct::COL_SKU]], $relation->getData('sku_parent'))) {
                    ...
                    if (count($result['configurable_variation_labels'])) {
                        ...
                        try {
                            ...
                            //'color=Color,bs_size=Size,bs_width=Width';
                            $configurable_variation_labels = [];
                            foreach ($variationAttributeCode as $item) {
                                if (isset($result['configurable_variation_labels'][$item])) {
                                    $configurable_variation_labels[$item] = $result['configurable_variation_labels'][$item];
                                }
                            }
                            $value = urldecode(http_build_query($configurable_variation_labels, '', ','));
                        } catch (\Exception $e) {
