...
class SeoDataObserver implements ObserverInterface
{
    ...
    /**
     * @var \Mirasvit\Seo\Api\Service\WidgetGeneratorServiceInterface
     */
     ...
     * @var string
     */
    protected static $productDescriptionOutputHtml;

    /**
     * SeoDataObserver constructor.
    ...
    public function productAttribute($outputHelper, $outputHtml, $params)
    {
        ...
        if ($params['attribute'] == 'description' && self::$productDescriptionOutputHtml) {
            return self::$productDescriptionOutputHtml;
        } elseif ($params['attribute'] == 'description' && $this->isProductDescriptionPrinted) {
            return $outputHtml;
        }
        ...
        switch ($params['attribute']) {
            ...
            case 'short_description':
                ...
                if ($shortDescription = $seo->getShortDescription()) {
                    $shortDescription = $this->widgetGeneratorService->generateWidget($shortDescription);
                    $outputHtml = $this->catalogOutput->productAttribute(
                        $currentProduct, $shortDescription, 'short_description'
                    );
                }
            ...
            case 'description':
                ...
                if ($fullDescription = $seo->getFullDescription()) {
                    $fullDescription = $this->widgetGeneratorService->generateWidget($fullDescription);
                    $outputHtml = $this->catalogOutput->productAttribute(
                        $currentProduct, $fullDescription, 'description'
                    );
                }
                ...
                self::$productDescriptionOutputHtml = $outputHtml;
                break;
        }
