...
class ImagePathPlugin
{
    ...
    private function getFriendlyImagePath($productData, $url)
    {
        ...
        $isEnableImageFriendlyUrl = $this->imageConfig->isEnableImageFriendlyUrl();

        if ($isEnableImageFriendlyUrl == ImageConfigServiceInterface::IMAGE_ENABLED_OLD_VERSION) {
            $imagePath =  $product->getId() . substr(md5($productData['image_id'] . $cacheUrl), 4, 3);
        } else {
            $imagePath =  $product->getId() . substr(md5($cacheUrl), 4, 4);
        }
