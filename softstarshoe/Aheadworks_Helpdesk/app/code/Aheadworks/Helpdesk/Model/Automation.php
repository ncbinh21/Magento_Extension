...
class Automation extends \Magento\Framework\Model\AbstractModel
{
    ...
    private function sendEmailToCustomer($template, $messageId = null)
    {
        ...
        }elseif(!empty($_POST['attachment'])){
            $connection = $this->getResource()->getConnection();
            $subQuery = $connection->select()->from(['ahtm' => $connection->getTableName('aw_helpdesk_ticket_message')],[
                new \Zend_Db_Expr('MAX(id)')
            ])->where('ticket_id = ?',$flatModel->getTicketId());
            $select = $connection->select()->from(['aha' => $connection->getTableName('aw_helpdesk_attachment')],[
                'id' => 'id',
                'name' => 'name',
                'length' => new \Zend_Db_Expr('length(content)'),
                'content' => 'content'
            ])->where('message_id = ?',new \Zend_Db_Expr('('.$subQuery.')'));
