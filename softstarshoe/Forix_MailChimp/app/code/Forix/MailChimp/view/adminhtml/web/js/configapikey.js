    ...
    function ($) {
        ...
        $.widget('mage.configmonkeyapikey', {
            ...
            _init: function () {
                ...
                $('#mailchimp_blog_monkeystore_blog').change(function () {
                    self._loadDetailsBlog();
                });
            },
            ...
            _loadStoresBlog: function (apiKey) {
                var self = this;
                var storeUrl = this.options.storeUrl;
                // remove all items in list combo
                $('#mailchimp_blog_monkeystore_blog').empty();
                // get the selected apikey
                $('#mailchimp_blog_monkeystore_blog').append($('<option>', {
                    value: -1,
                    text : 'Select one Mailchimp Store'
                }));
                $('#mailchimp_blog_monkeylist_blog').append($('<option>', {
                    value: -1,
                    text : 'Select one Mailchimp Store'
                }));
                // get the list for this apikey via ajax
                $.ajax({
                    ...
                    if ($('#mailchimp_blog_monkeystore_blog option').length>1) {
                        $('#row_mailchimp_blog_monkeystore_blog').find('.note').hide();
                    } else {
                        $('#row_mailchimp_blog_monkeystore_blog').find('.note').show();
                    }
                    self._loadDetailsBlog();
                });
            },
            ...
            _loadDetailsBlog: function () {
                var detailsUrl = this.options.detailsUrl;
                var selectedApiKey = $('#mailchimp_general_apikey').find(':selected').val();
                var selectedStore = $('#mailchimp_blog_monkeystore_blog').find(':selected').val();
                $('#mailchimp_blog_account_details_ul_blog').empty();
                $('#mailchimp_blog_monkeylist_blog').empty();
                $.ajax({
                    url: detailsUrl,
                    data: {'form_key':  window.FORM_KEY, 'apikey': selectedApiKey, "store": selectedStore},
                    type: 'POST',
                    ...
                    $.each(data, function (i,item) {
                        if (item.hasOwnProperty('label')) {
                            $('#mailchimp_blog_account_details_ul_blog').append('<li>' + item.label + ' ' + item.value + '</li>');
                        }
                    });
                    $('#mailchimp_blog_monkeylist_blog').append($('<option>', {
                        value: data.list_id,
