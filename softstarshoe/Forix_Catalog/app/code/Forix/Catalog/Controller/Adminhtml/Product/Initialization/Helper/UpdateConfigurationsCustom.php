 ...
 */
namespace Forix\Catalog\Controller\Adminhtml\Product\Initialization\Helper;

class UpdateConfigurationsCustom
{
    /**
        ...
        if (isset($configurableMatrix) && $configurableMatrix != "") {
            ...
            foreach ($configurableMatrix as $item) {
                if (isset($item['media_gallery'])) {
                    $position = 1;
                    foreach ($item['media_gallery']['images'] as $key => $value) {
                        unset($item['media_gallery']['images'][$key]['position']);
                        $item['media_gallery']['images'][$key]['position'] = $position;
                        $position++;
                    }
                }
