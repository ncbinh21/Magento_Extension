...
use Yosto\InstagramConnect\Helper\Exception\AuthException;
use Yosto\InstagramConnect\Helper\Exception\CurlException;
use Yosto\InstagramConnect\Helper\Exception\InvalidParameterException;
use Yosto\InstagramConnect\Helper\Exception\PaginationException;
...
class InstagramClient {
    ...
    public function __construct(Data $helper) {
        $this->_helper = $helper;
        $this->setAccessToken($this->getInstagramAccessToken());
        // if you want to access user data
        $config = [
            'apiKey' => $this->getInstagramClientIdConfig(),
            'apiSecret' => $this->getInstagramClientSecretConfig(),
            'apiCallback' => $this->_helper->getBaseUrl(),
            'scope' => $this->_availableScope
        ];
     ...
     * @return mixed
     */
    public function getSelfFollowedBy($limit) {
    ...
    public function getOAuthTokenFromAjaxPost(
        $code,
        $clientId,
        $clientSecret,
        $redirectUri
        ...
        if (isset($result->code) && $result->code != 200) {
            return json_encode(["error_type"=> $result->error_type, "error_message" => $result->error_message]);
        }
    ...
    {
        return $this->_helper->getInstagramClientIdConfig();
    }

    public function getInstagramClientSecretConfig()
    {
        return $this->_helper->getInstagramClientSecretConfig();
    }

    public function getIsIndexDisplayConfig()
    {
        return $this->_helper->getIsIndexDisplayConfig();
    }

    public function getIsProductDisplayConfig() {
        return $this->_helper->getIsProductDisplayConfig();
    }
    ...
    {
        return $this->_helper->getInstagramAccessToken();
    }
    public function getIsDisplayLikesComments()
    {
        return $this->_helper->getIsDisplayLikesComments();
    }
    public function getProductDetailImageNumber()
    {
        return $this->_helper->getProductDetailImageNumber();
    }

    public function getProductDetailPageUrl($_productId)
    {
        return $this->_helper->getProductDetailPageUrl($_productId);
    }

    public function getShoppingPageUrl()
    {
        return $this->_helper->getShoppingPageUrl();
    }

    public function getShoppingPageLinkMenu()
    {
        return $this->_helper->getShoppingPageLinkMenu();
    }

    public function getShoppingPageLinkLabel()
    {
        return $this->_helper->getShoppingPageLinkLabel();
    }

    public function getShoppingPageTemplate()
    {
        return $this->_helper->getShoppingPageTemplate();
    }
