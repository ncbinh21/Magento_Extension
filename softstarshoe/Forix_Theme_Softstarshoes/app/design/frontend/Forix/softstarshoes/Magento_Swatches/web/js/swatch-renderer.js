...
], function ($, _, mageTemplate, keyboardHandler, $t, priceUtils, resolver) {
    'use strict';
    ...
    $.widget('mage.SwatchRenderer', {
        ...
        _create: function () {
            var options = this.options,
                ...
                $main = productData.isInProductView ?
                    ...
                    this.element.parents('.product-item-info .product-item-photo');
        ...
        _loadMedia: function (eventName) {
            var $main = this.inProductList ?
                    this.element.parents('.product-item-photo') :
                    this.element.parents('.column.main'),
                images;
        ...
        _OnClick: function ($this, $widget, eventName) {
            ...
            if ($widget.inProductList) {
                ...
                var $parent2 = $this.parents('.product-item-info');
                var suffixText = attributeId + '=' + $this.attr('option-id');
                var oldLink = $parent2.find('.product').attr('href');
                var arr = oldLink.split('#');
                var newLink = arr[0] + '#' + suffixText;
                $parent2.find('.product').attr("href", newLink);
            }
        ...
        _LoadProductMedia: function () {
            ...
            mediaCallData = {
                ...
                'isInCategoryPage': !productData.isInProductView
            };
        ...
        _EmulateSelected: function (selectedAttributes) {
            var $widget = this;
            var defaultValues = {};
            if($widget.options.jsonConfig.defaultValues !== undefined){
                $.each($widget.options.jsonConfig.attributes, function (key, value) {
                    defaultValues[value['code']] = $widget.options.jsonConfig.defaultValues[value['id']];
                });
                selectedAttributes = $.extend({}, defaultValues, selectedAttributes);
            }
            resolver(function(){
                $.each(selectedAttributes, $.proxy(function (attributeCode, optionId) {
                    var optionElm = $widget.element.find('.' + $widget.options.classes.attributeClass +
                        '[attribute-code="' + attributeCode + '"] [option-id="' + optionId + '"]');
                    if (optionElm.is('option') ) {
                        optionElm.parent('select').val(optionId).trigger('change');
                    } else if (!optionElm.hasClass('selected')) {
                        optionElm.trigger('click');
                    }
