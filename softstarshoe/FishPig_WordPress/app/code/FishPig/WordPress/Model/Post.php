...
class Post extends \FishPig\WordPress\Model\Meta\AbstractMeta implements ViewableInterface
{
	...
	**/
	...
	{
		$tags = $this->getTermCollection('post_tag');
		$tags = $tags->getColumnValues('name');
		$postDate = $this->getPostDate('Y-m-d H:h:i');

		$posts = $this->_factory->getFactory('Post')->create()->getCollection();
		$posts->addPostTypeFilter('post')
		->addTermFilter($tags, 'post_tag', 'name')
		->addFieldToFilter('ID',['nin' => $this->getId()])
		->addFieldToFilter('post_date',['lt' => $postDate])
