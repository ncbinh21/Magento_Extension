<?php
namespace Migration\Step\ForixGiftcard;
...
class Data extends \Migration\Step\DatabaseStage implements StageInterface
{
    ...
    public function __construct(
        ...
        $this->map = $mapFactory->create('forix_giftcard_map_file');
        $this->progress = $progress;
        $this->readerGroups = $groupsFactory->create('forix_giftcard_groups_file');
        $this->logger = $logger;
        $this->helper = $helper;
        $this->config = $config;
    ...
    public function perform()
    {
        ...
        foreach ($sourceDocuments as $sourceDocName) {
            ...
            while (!empty($items = $this->source->getRecords($sourceDocName, $pageNumber))) {
                ...
                foreach ($items as $recordData) {
                    ...
                    $where= [];
                    foreach($fieldsUpdateOnDuplicate as $_field){
                        $where["$_field = ?"] = $destRecord->getValue($_field);
                    }
                    $destinationAdapter->delete($destTable,$where);
        ...
        $destinationAdapter->truncateTable('aw_giftcard_history_entity');
        $destinationAdapter->truncateTable('aw_giftcard_invoice');
        $destinationAdapter->truncateTable('aw_giftcard_order');
        $destinationAdapter->truncateTable('aw_giftcard_statistics');
