...
class Queue extends AbstractModel implements QueueInterface
{
    ...
    {
        $this->_init('Mirasvit\Email\Model\ResourceModel\Queue');
    }
     ...
     */
    public function getTriggerChain()
    {
        if ($this->triggerChain == null) {
            $this->triggerChain = $this->chainFactory->create()
    ...
    {
        return $this->getTriggerChain()->getTemplate();
    }
    ...
    public function getArgs($key = null)
    {
        if ($this->args == null) {
            ...
            $this->args['chain'] = $this->getTriggerChain();
    ...
    {
        if (!$this->getSubject()) {
            $this->setSubject($this->getTemplate()->getProcessedTemplateSubject($this->getArgs()));
    ...
    {
        if (!$this->getContent()) {
            $this->setContent($this->getTemplate()->getProcessedTemplateText($this->getArgs()));
        }
