...
class AbandonedEvent extends CronEvent
{
    ...
    public function execute()
    {
        ...
        /** select carts, that updated between 1hour..30 minutes ago */
        $updatedAtFrom = $this->context->timeService->shiftDateTime(60 * 60, $lastCheck);

        $updatedAtTo = $this->context->timeService->shiftDateTime(30 * 60, $lastCheck);
        ...
        $select = $connection->select()
            ...
            ->group('q.entity_id')
            ->having(
                '(GREATEST(q.updated_at, q.created_at) > ? OR MAX(GREATEST(qi.updated_at, qi.created_at)) > ?)',
                $updatedAtFrom
            )
            ->having(
                '(GREATEST(q.updated_at, q.created_at) < ? OR MAX(GREATEST(qi.updated_at, qi.created_at)) < ?)',
        ...
        foreach ($quotes as $quote) {
            $params = [
                ...
                self::PARAM_CREATED_AT     => max($quote['updated_at'], $quote['created_at']),
            ];
